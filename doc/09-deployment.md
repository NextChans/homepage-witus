...
이 도메인은 가비아 등록이고 네임서버도 가비아(`ns.gabia.co.kr`)다.

> ⚠️ **함정: 레코드가 0개인데 도메인이 응답한다.**
> 가비아의 **포워딩 / 웹 파킹 / 모바일 파킹** 서비스는 DNS 레코드와 별개로
> **네임서버 단에서 가비아 IP 를 내려준다.** 실제로 `witus.kr` 은 레코드 0개
> 상태에서 `121.254.178.237` 로 응답하며 `<frameset>` 안에 Vercel 프리뷰 주소를
> 끼워 넣고 있었다(마스킹 포워딩).
>
> **이 상태로 A 레코드를 추가해도 충돌한다.** 먼저 끈다.

1. **DNS 관리 → 하단 `도메인 연결 설정`**
   - `포워딩` → **해제**
   - `웹 파킹` · `모바일 파킹` 도 켜져 있으면 **해제**
   - 해제 직후 도메인은 아무 곳에도 연결되지 않는다 — **정상이다.**
2. **`DNS 설정` → `레코드 수정`** 으로 레코드 2개 추가
...
⚠️ **가비아는 `ALIAS`/`ANAME` 을 제공하지 않는다**(A/AAAA/CNAME/MX/TXT/SRV). 따라서
apex 는 **A 레코드 고정**이고, Vercel 이 IP 를 바꾸면 **수동으로 고쳐야 한다.**
apex 를 정규로 택한 비용이다(ADR-025). 타입 드롭다운에서 다시 확인할 것.
