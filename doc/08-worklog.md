...
### 상호에서 `주식회사` 제거 + 제휴사 표기 내림 (ADR-034, ADR-035)

**`legalName: '주식회사 위투스' → 'WITUS'`** (사용자 결정). 설립등기 전인데
상법 제20조가 **회사가 아니면 상호에 회사임을 표시하는 문자를 쓰지 못하게** 한다.
ADR-028(플레이스홀더 제거)과 같은 판단인데, `000-00-00000` 은 누가 봐도 가짜였고
`주식회사` 는 **그럴듯해서 아무도 의심하지 않는다** — 그래서 더 위험하다.

**`features.partnerLogos: false`** — 제휴 확정 전이다. `metrics` 와 같은 이유에
두 가지가 더 붙는다: "연동합니다" 는 **제휴 관계를 주장하는 문장**이라 실적 없는
곳을 올리면 표시광고법 리스크고, 실명 게재에는 **각 사 서면 동의**가 선행된다.

### 여기서 잡은 결함 — 빈 래퍼

플래그를 끄자 `LogoStrip` 은 `null` 을 반환했는데 **호출부가 감싸던
`border-t bg-surface` div 가 그대로 남아** 높이 1px 짜리 띠가 CTA 밴드 위에
hairline 을 두 줄로 만들었다. 스크린샷으로는 놓칠 수준이라 **DOM 을 재서** 찾았다.

→ 래퍼를 컴포넌트 안으로 옮겼다. **숨길 수 있는 섹션은 배경·구분선 같은 자기
크롬을 자기가 소유해야 한다.** 같은 실수를 반복하기 쉬워 `app/page.tsx` 에 경고
주석을 달고 `homepage-verify` 체크리스트에 빈 래퍼 검사(DOM 측정 코드 포함)를 넣었다.
...
사용자 요청: "이 세션에서 사용한 모든 쿼리·스크립트 등 산출물들을 적절히 모아두라."

확인해 보니 **절반만 남아 있었다.** 마이그레이션과 적용 전후 점검 쿼리는 있었지만,
pg_cron 진단·잡 등록·수동 실행·**다음 날 확인 쿼리**는 문장으로만 설명돼 있고
실행 가능한 형태가 없었다. 채팅으로만 세 번 전달한 것이 그대로 증발할 뻔했다.

**새로 만든 것 3가지**

| 산출물 | 내용 |
|---|---|
| `doc/11-sql-runbook.md` | 실행 가능한 SQL 12블록. 마이그레이션 점검 → cron 등록·진단 → 파기 잡 확인 → 보관기간 연장 |
| `doc/12-build-from-scratch.md` | 빈 프로젝트 → 현재까지 11단계. 각 단계의 **함정**이 본문이다 |
| `scripts/verify/` | 가짜 Slack(4599) · 가짜 Supabase(4600) · Apps Script 테스트 15개 + README |

**`scripts/verify/` 가 특히 급했다** — 이 스텁들은 컨테이너 임시 폴더에만 있어서
**세션이 끝나면 사라질 상태였다.** 검증 스킬에는 "이렇게 만들면 된다" 고 산문으로만
적혀 있었는데, 그러면 다음 사람이 매번 새로 쓴다. 저장소로 옮기고
`npm run verify:mail` 로 부를 수 있게 했다. 옮긴 뒤에도 15개 검사가 그대로 돈다.

**원칙 — 원본은 한 곳에만 둔다.** 스키마 생성 SQL 은 `supabase/migrations/` 가
원본이고 런북에 복사하지 않았다. 복사하면 갈린다. 런북은 **운영·진단 쿼리만** 담는다.

`doc/README.md` 에 "문서 밖의 산출물" 표를 추가해 마이그레이션·브랜드 자산·
Apps Script·검증 스텁·스킬이 각각 어디 있는지 한눈에 보이게 했다.

검증 스킬의 함정 14 도 "스텁을 이렇게 쓰라" 에서 **"`scripts/verify/` 를 쓰라"** 로
바꿨다. 매번 새로 쓰게 두면 안 쓰게 된다.

### 자가진단 도구 (ADR-036, ADR-037)

"홈페이지에서 제공할 미끼 상품" 요청 → **전자금융업 등록 대상 자가진단**(`/check`).

**개인정보를 받지 않는 설계**라 처리방침이 비공개인 지금도 켤 수 있다. 답변은 URL
쿼리스트링에만 있고 저장하지 않는다. `<form method="get">` + 라디오라 **JS 없이**
동작하고, 결과 URL 을 그대로 공유할 수 있다.

**결정 트리가 아니라 독립 판정 다발**이다. 하나의 서비스가 여러 업종에 동시에
해당하므로(PG + 선불 + 에스크로) 트리로 만들면 둘을 놓친다.

### 조사로 초안을 세 군데 고쳤다

사용자가 "찾아봐" 해서 법령을 조사했고, **제 초안이 틀린 곳이 나왔다.**

- 선불 면제는 **발행잔액 30억 AND 연 총발행액 500억** — 둘 다 밑돌아야 면제.
  2차 출처 하나가 `또는` 으로 적었는데 금융위 보도자료·법무법인 자료는 `및` 이다.
  **면제 범위가 완전히 달라지는 차이라** 교차 확인이 아니었으면 그대로 틀렸다
- 선불 기준이 업종이 아니라 **가맹점 수**다. 10개 이하 → **1개**만 면제
- PG 는 자금 흐름만으로 판정 불가 — **부수성**을 물어야 한다

**그리고 3개월 뒤 시행되는 개정을 찾았다.** 2025. 12. 16. 공포 / **2026. 12. 17. 시행** —
PG 정산자금 외부관리 의무, PG 자본금 강화, 대주주 변경승인, PG 정의 변경(중개업
부수 정산 제외). 마지막 것 때문에 **같은 사업도 시점에 따라 판정이 갈려서**,
결과를 `현재` / `2026. 12. 17. 이후` 로 나눠 보여준다. 이게 도구의 차별점이 됐다.

### 플래그는 끈 채로 머지한다

`eligibilityCheck: false`. 다른 플래그와 **위험의 성격이 다르다** — `metrics` 는
꺼도 아쉬울 뿐이지만 이건 **틀린 채로 켜면 사업에 해가 된다.**
법제처가 SPA 라 원문 추출에 실패해 2차 출처 종합으로만 채웠고, 켜기 전 조건
(법령 원문 대조·시행령 확인·기준일 갱신·면책 고지 유지)을 주석에 적었다.

**검증** 판정 분기 6종을 실제 요청으로 확인했다 — PG 주업/부수(2026 뒤집힘),
선불 자가형/1곳/2곳+, 규모 AND 조건(하나만 넘어도 대상). **JS 를 끈 브라우저**에서
폼 제출·결과 렌더까지 확인했고, 1280/390 가로 넘침 없음.

초안에 붙였던 `dynamic = 'force-static'` 은 걷어냈다 — `searchParams` 를 읽는
페이지는 정적일 수 없고, 강제하면 쿼리가 빈 채로 들어와 결과가 안 나온다.

### 플래그로 숨긴 페이지의 메타데이터 누출 수정 (ADR-038)

`/check` 를 배포하고 프로덕션에서 확인하다 찾았다. **404 는 맞는데 응답 본문에
페이지 제목·설명이 그대로 실려 있었다.** `export const metadata` 가 정적 export 라
컴포넌트의 `notFound()` 보다 먼저 평가되기 때문이다. `/privacy` 도 같았다 —
기존부터 있던 패턴 결함이다.

→ 둘 다 `generateMetadata()` 로 바꿔 플래그가 꺼져 있으면 빈 객체를 돌려준다.

보안 문제는 아니지만 **숨긴 의미가 없어진다.** 특히 `/privacy` 는 처리방침을 아직
공개하지 않기로 한 판단과 앞뒤가 맞지 않는다.

**검증 중 또 같은 실수를 할 뻔했다.** 첫 측정에서 서버가 안 떠 `000` 이 나왔는데
"누출 0회" 가 함께 찍혔다. 그대로 믿었으면 고치지 않은 것을 고쳤다고 판단했을
것이다. → 이후 측정에 **본문 길이를 함께 찍어** "빈 응답이라 0" 과 "내용이 있는데
0" 을 구분했다. 이 세션에서 **네 번째** 같은 형태의 실패다.

### 자가진단을 법령 원문과 대조 (ADR-039)

사용자가 법제처 PDF 3건(법률 제21205호 2025.12.16 / 2026.12.17 시행, 시행령
제36281호)을 올려 주면서 그동안 못 하던 원문 대조를 했다. 법제처는 SPA 라
`curl`·`WebFetch` 모두 껍데기만 온다 — 이번에도 세 경로로 다시 확인하고 실패했다.

**오류 4건, 누락 3건.** 전부 "숫자는 맞는데 붙는 자리가 틀렸다" 였고, 그중 하나는
판정이 **정반대**였다 — 2026. 12. 17. 개정은 중개업 부수 정산을 제외하는 게 아니라
**정보만 전달하는 PG 의 등록 면제 조항(법 제28조③2)을 삭제**한다. 지금 면제받는
쪽이 등록 대상이 된다.

그래서 PG 판정을 두 축으로 나눴다 — 해당 여부는 결제정보 송·수신(법 제2조19호),
면제 여부는 자금 수수(영 제15조⑦). 초안은 하나로 둘 다 판단해서 정보만 전달하는
사업자를 아예 "해당 없음" 으로 떨어뜨리고 있었다. 이 도구에서 제일 쓸모 있는 답을
못 내고 있었던 셈이다.

`Verdict.lawRef` 를 추가해 근거 조문을 화면에 낸다. 위 오류들이 잡히지 않았던 이유가
**조문 번호가 없어서**였다.

고지 첫 줄은 사용자 요청으로 "법무 검토를 받지 않은 단순 참고용입니다" 로 바꿨다.
조문이 붙으면서 오히려 확정된 답처럼 읽히게 됐기 때문이다.

플래그 쓰기가 막혀 있어 `evaluate()` 를 `node --experimental-strip-types` 로 직접
돌려 검증했다. 그 과정에서 화면에 나가는 문자열에 마크다운 `**` 이 남아 있는 것도
찾았다 — 이 페이지는 마크다운을 파싱하지 않는다.

### 고시까지 받아 질문을 조문에 맞춤 (ADR-040)

ADR-039 가 남긴 "변호사가 아니라 원문이 더 필요한 것" 3건 중 2건이 전자금융감독규정
(금융위원회고시 제2026-29호)으로 풀렸다.

이번엔 판정이 아니라 **질문이 틀렸다.** 조문의 요건을 일부만 묻고 있어서, 답하는
사람이 다른 것을 생각하고 답하면 결과가 같이 틀어지는 상태였다.

제일 위험했던 건 거래금액이다. "분기별 전자금융거래 총액" 으로 물었는데 고시 §42의2①의
기준은 **업종별 금액**이고 결제대행금액은 **이용자가 지급한 재화·용역의 매출총액**이다.
PG 담당자가 수수료 수익으로 답하면 자릿수가 통째로 틀린다.

나머지 넷 — 결제대금예치의 전자상거래법 한정, 전자고지결제의 3요소, 전자자금이체의
추심이체 포함, 선불 발행잔액의 분기말 단순평균 — 도 같이 조였다.

법 §30④(소규모 등록 후 2분기 연속 초과)을 현행 의무에 추가했다. 기한이 "금융위원회가
정하는 기한" 이라 법률만으로는 알 수 없었는데 고시 §42의2②에 6월로 있다.

**남은 건 법 §2⑭의 특수관계인 범위다.** 시행령에도 고시에도 위임 규정이 없다.
계열사에서 쓰이는 포인트가 지금 도구에서는 등록 대상으로 떨어지는데, 특수관계인
범위에 들면 아닐 수 있다. 확인 전까지 안내로 "모르겠다" 를 유도한다 — 없는 의무를
있다고 말하는 쪽이 더 나쁘다.

### 자가진단의 "해당 없음" 이 막다른 길이었다 (ADR-041)

사용자 사업 구조(밴대리점 · PG 영업대행 · 자기 명의 정산 없음)를 확인하다 두 가지가
나왔다.

첫째, ADR-040 이 법무 검토 항목으로 남긴 "VAN 승인 전문 중계" 는 **이미 답이
있었다.** 고시 §3①1호가 그걸 전자금융보조업자로 분류해 둔다. 같은 고시를 읽고도
§42(금액)만 찾고 §3(범위)을 안 봐서 놓쳤다.

둘째가 더 중요하다. 밴대리점·영업대행은 자금도 결제정보도 만지지 않아 이 도구에서
전부 "해당하지 않는 것으로 보입니다" 가 나온다. 그런데 여전법에 가맹모집인 등록이
따로 있다(원문 미대조). **도구가 안심시키는 쪽으로 작동하고 있었다.**

판정 도구는 "아니오" 를 낼 때가 더 위험하다. "예" 는 사람을 찾아가게 하지만 "아니오"
는 거기서 끝난다. 고지에 "「전자금융거래법」 기준" 이라는 범위 한 줄을 넣었다 —
여전법 내용을 몰라도 참인 문장이다.

### 여전법 대조 — WITUS 자신의 규제 지위 (ADR-042, doc/13)

사용자가 여신전문금융업법(법률 제21065호)·시행령(대통령령 제36314호) PDF 를 올려
주어 ADR-041 이 "원문 미대조" 로 남긴 것을 확정했다.

결론: **전자금융거래법상 아무 지위도 아니고, 여신전문금융업법의 가맹점모집인이다.**

앞서 몰랐던 것 넷 중 둘이 컸다. 첫째, 등록 신청 주체가 **부가통신업자(VAN 사)** 다
(법 §16의3①) — "우리가 금융위에 신청한다" 로 알고 움직였으면 헛걸음했다. 둘째,
**재위탁이 금지**된다(영 §6의10①4호) — 밴 영업이 통상 다단계 대리점으로 돌아가는 걸
감안하면 사업 모델 확인이 필요하다.

그리고 **사업자등록이 영업 자체의 전제**였다(영 §6의10④). 지금까지는 홈페이지 표기
때문에 기다리는 것으로만 보고 있었다.

자가진단 도구(방문자용)와 성격이 달라 `doc/13-witus-regulatory-status.md` 로 뺐다.

`content/services.ts` 의 "VAN 가맹점 등록 대행" 문안은 **유지**하기로 했다(사용자
판단). 등록 순서상 지금 앞당길 수 있는 단계가 없고, 제재가 우리에게 직접 오지 않으며,
VAN 사가 미등록 상태로는 계약을 안 받아주므로 실무 제약이 먼저 오기 때문이다.
등록 완료 후 등록번호를 `content/site.ts` 에 넣는 것이 남는다.

### PG 사 영업대행은 여전법 §16의2 밖이다 (doc/13 갱신)

사용자가 "PG 쪽은 사실상 소개 정도" 라고 확인해 주어 다시 봤는데, 결론을 가른 것은
**소개냐 중개냐가 아니었다.**

법 §2 5호가 신용카드가맹점을 두 갈래로 정의한다 — 가목(신용카드업자와 계약해 물품
판매·용역 제공)과 나목(신용카드업자와 계약해 거래를 대행하는 자 = 결제대행업체).
**PG 사 자신이 나목 신용카드가맹점**이고, PG 의 하위 쇼핑몰은 PG 사와 계약하지
신용카드업자와 직접 계약하지 않으므로 **신용카드가맹점이 아니다.**

§16의2① 은 "신용카드가맹점을 모집할 수 있는 자" 다. 대상이 아닌 것을 소개하는 것은
조문 밖이라 행위 성격을 따질 필요가 없었다.

→ 남은 질문이 하나로 좁혀졌다. VAN 쪽의 "가맹점 등록 대행" 이 법 §2 5의3 의
  "가맹점계약의 체결을 중개 또는 대리" 인가. 오프라인 매장은 카드사와 직접 계약하므로
  가목 가맹점이고, 단말기 설치는 이미 하고 있으니 두 요소가 다 채워질 수 있다.

### 배포 확인 스크립트의 측정 결함 (homepage-verify 함정 7-1)

프로덕션 회귀 확인에서 `/contact 200 본문 0 B`, `/privacy 404 본문 0 B` 가 각각
한 번씩 나왔다. 둘 다 한 번의 요청에서 코드·크기를 함께 받아 3회 재측정하니 정상이었다
(28,568 B / 16,287 B).

원인은 스크립트가 **본문과 상태코드를 각각 다른 curl 로 받고** 있었던 것이다. 배포
전환 중에는 두 요청이 다른 응답을 본다.

ADR-038 은 "빈 값을 정상 데이터로 읽는" 실패를 기록했는데, 이건 **정반대 방향**이다 —
정상을 장애로 읽는다. 한 번은 넘겼지만 두 번 나와서 스킬에 고쳤다.

### 가맹점모집인 판단을 뒤집었다 (ADR-043)

사용자가 "신용카드업자를 위해 가맹점계약 체결을 중개 또는 대리하지 않는다" 고 확인해
주어 doc/13 의 결론을 정정했다. 여전법 §2 5의3 은 두 요소를 `하고` 로 묶어 모두
요구하므로 단말기 설치만으로는 가맹점모집인이 아니다. **여전법상 등록 의무 없음.**

**무엇을 틀렸나 — 홈페이지 문안을 사실관계로 읽었다.** 직전까지 "실무적으로는 해당한다고
보는 쪽이 안전" 이라고 적었는데, 그 유일한 근거가 `content/services.ts` 의 "카드사 가맹
등록과 VAN 사 단말 등록을 병행 진행" 이었다. 마케팅 문안은 넓게 쓰이고 실제 업무는 그보다
좁은 것이 보통이다 — 규제 판단의 입력으로 쓸 자료가 아니었다.

ADR-039 가 공개 자료를 원문 대신 쓴 것과 같은 계열이다. **둘 다 옆에 있는 것을 근거로
삼았다.**

doc/13 에 "다시 봐야 하는 신호" 를 넣었다 — 가맹 신청 서류 접수·제출 관여, 카드사
직가맹·오프라인 결제 병행, 자기 명의 정산. 결론만 남기면 조건을 모른 채 인용된다.

남은 것은 문안과 실제 업무의 간극인데, 이건 등록 의무가 아니라 표시 문제이고 사업
표현이라 사용자가 정한다.

### 밴 단말기 문안을 실제 업무에 맞춤 (ADR-044)

사용자 확인: **카드사 가맹 등록은 밴사가 한다.** ADR-043 의 "중개·대리를 하지 않는다"
와 합치면 문안이 남의 일을 우리 산출물로 적고 있었다.

네 곳을 고쳤다 — 산출물 제목·본문, 프로세스 03, 그리고 `intro` 의 "VAN 사 등록"
(가맹 등록으로 읽혀서 "VAN 사 단말 등록" 으로 좁혔다).

제목을 "VAN 가맹점 등록 대행" 에서 **"개점 일정 맞춤 설치"** 로 바꾼 것이 핵심이다 —
수단이 아니라 **결과**로 적으면 사실과 맞으면서 가치는 그대로다.

ADR-042 의 "유지" 판단을 뒤집은 것인데, 그 판단이 전제(가맹점모집인일 수 있다 /
가맹 등록 주체 미확인)를 함께 적어 두었기 때문에 깔끔하게 뒤집을 수 있었다.
**전제를 적어두지 않으면 무엇이 바뀌었는지 알 수 없다.**

검증은 로컬 렌더로 옛 문안 4종 0회를 확인하고 데스크톱·모바일 스크린샷까지 봤다.

### 자가진단 공개 전 확인 3건을 doc/06 에 기록

사용자가 "법무 검토 3건 뭔데?" 라고 물어 확인해 보니 **3건 중 2건이 어디에도 적혀
있지 않았다.** 채팅으로만 말했다. 컨테이너는 회수되므로 사라질 뻔했다.

`doc/06-security-compliance.md` 의 "현재 비활성 상태인 기능" 표에 `/check` 를 추가하고,
`inquiryForm` 과 같은 형태로 "켜기 전 확인할 3건" 블록을 넣었다.

**정리하면서 알게 된 것 — 셋의 성격이 서로 다르다.**

| # | 무엇 | 누가 답하나 |
|---|---|---|
| 1 | 법 §2⑭ 특수관계인 범위 | 금융당국 (법령해석 신청) |
| 2 | 비전문가가 질문을 조문대로 읽는가 | 실제 사용자 (테스트) |
| 3 | 변호사법 §109 에 닿는가 | 변호사 |

"법무 검토 3건" 으로 뭉뚱그려 부르고 있었는데 **변호사가 답할 것은 3번 하나**다.
1번은 당국 질의, 2번은 사용자 테스트다. 이름을 잘못 붙이면 엉뚱한 데 시간을 쓴다.

### 판정을 걷어내고 조문 문언 매칭으로 전환 (ADR-045)

사용자 지시로 `/check` 의 성격을 바꿨다. "귀사는 등록 대상입니다" 라는 결론을 내지 않고,
답변이 **어느 정의 문언에 대응하는지**를 조문 원문과 나란히 보여준다. 그리고
"등록하려면 무엇이 필요한가"(요건·서류·기한·정보기술 8개 부문)를 정보로 낸다.

**작업 중에 이 파일이 정반대의 판정을 내보내고 있던 것을 발견했다.** 법 제2조19호가
2026. 12. 17. 에 좁아진다 — "지급결제정보를 송신·수신" 문언이 빠지고 통신판매중개 등
부수적 정산이 명시적으로 제외된다. 제28조③2호(면제) 삭제는 면제를 없애려는 것이 아니라
**정의가 좁아져 면제가 불필요해졌기 때문**이다.

ADR-039 는 제28조만 보고 "면제가 사라지니 대상이 된다" 고 적었다. 정반대였다. 그리고
ADR-039 가 오류라고 정정했던 초안의 "중개업 부수 정산 제외" 는 **맞았다** — 오류라고
판단한 것이 오류였다.

**조문 원문을 화면에 그대로 내는 것이 이 개편의 핵심**인 이유가 여기 있다. 원문이
있으면 이 도구가 틀려도 읽는 사람이 대조해서 안다. 판정만 내면 틀린 판정이 그대로 나간다.

화면 확인은 못 했다 — features 플래그 쓰기가 차단돼 있다. 레이아웃이 크게 바뀌었으므로
플래그를 켜기 전에 눈으로 봐야 한다.

### 공개 전 확인 3건 정리 — 하나는 코드로 막고 둘은 내려둔다 (ADR-046)

사용자가 공개 전 확인 3건을 하지 않겠다고 했다. 그 목록은 **판정 도구였을 때** 만든
것이고 ADR-045 로 성격이 바뀌었으니 타당한 판단이다.

**1번(특수관계인)은 당국 질의 없이 코드로 막았다.** `prepaidReach` 에 "계열사·특수관계인
에서만 쓴다" 선택지를 추가하고 `unsure` 로 받는다. 그 선택지가 없을 때는 계열사 포인트가
`others` 로 떨어져 "정의 문언에 대응합니다" 가 나왔다 — 과잉 감지였다.

**모르는 것을 묻지 않으면 아는 척하게 된다.** 선택지가 없으면 사용자는 가장 가까운 보기를
고르고 도구는 그걸 확정된 입력으로 받는다. "우리가 모르는 경우" 에도 선택지를 준다.

2번은 운영하면서 고치고, 3번(변호사법)은 받지 않는다 — 다만 "위험이 없다" 가 아니라
"줄었다" 까지가 확인된 것이라는 구분을 문서에 남겼다.

`doc/06` 의 체크리스트는 **결정 기록으로 바꿨다.** 안 하기로 한 것을 "확인 필요" 로
남겨두면 다음에 그 문서를 여는 사람이 같은 자리에서 또 막힌다. 질의문·연락처는 보존했다.

검증은 플래그를 임시로 켜서 화면까지 봤다(사용자가 features.ts 쓰기를 허용). 모바일에서
선택지 5개가 세로로 스택되고 가로 스크롤은 없다.

### /check 히어로 lede 를 한 문장으로 (ADR-045 후속)

데스크톱에서 4줄이라 히어로치고 길었다. 원인은 세 가지를 한 문단에 넣은 것이다 —
무엇을 하는가 / 판단은 당국이 / 개인정보 없음.

히어로에는 첫 번째만 남겼다. 나머지 둘은 고지 박스로 옮겼는데, "감독당국이 판단" 은
이미 있었으므로 실제로 옮긴 것은 개인정보 문구다.

⚠️ 개인정보 문구를 **지우지 않고 옮긴** 이유가 있다. 이 페이지가 `privacyPolicy` 플래그와
독립해서 켜질 수 있는 근거가 그 약속이기 때문이다. 주석으로 남겼다.

결과: 데스크톱 4줄 → 2줄, 모바일 3줄. 가로 스크롤 없음.

### 파기 잡 자동 실행 확인 + 테스트 건 2건 정리 (ADR-047)

**파기 잡 자동 실행이 확인됐다.** 등록 다음 날 `triggered_by = 'cron'` 행이 두 테이블
모두 생겼다 — `inquiries` 18:10 UTC, `admin_audit_log` 18:30 UTC. `deleted_count` 가
0인 것은 경과 건이 없어서고, `retained_count` 가 0인 것은 그것이 **보존 예외로 건너뛴
건수**여서다(전체 보관 건수가 아니다). **행의 존재 자체가 증거다.**

**그 김에 보관 현황을 재 봤더니 `inquiries` 에 2건이 있었다.** 폼은 꺼져 있는데.
한 건은 관리자 수동 등록, 한 건은 **실제 폼 경로**로 들어온 것(`ip_hash` 까지 있었다).
사용자가 구축 중 직접 넣은 테스트로 확인돼 둘 다 정리했다.

**문서의 근거가 실제와 달랐다.** `doc/06` 은 처리방침 404 를 "폼을 내려 개인정보를
수집하지 않으므로" 로 정당화하고 있었는데, 개인정보보호법 제30조는 **수집이 아니라
처리(보관 포함)** 에 대해 처리방침을 요구한다. 테이블에 한 건이라도 있으면 성립하지
않는다. 문구를 정정하고 **확인 쿼리를 함께 넣었다** — 말로 적힌 전제는 조용히 거짓이
된다.

**내 실수도 하나.** 설명하면서 함수 본문 조각을 그대로 인용했더니 사용자가 SQL Editor 에
붙여넣어 42703 에러가 났다. PL/pgSQL 지역 변수는 함수 밖에서 안 산다. runbook 첫머리
함정 절에 추가했고, Supabase 린터가 이걸 "RLS 없는 테이블 생성" 으로 오인한다는 것도
같이 적었다(`Run and enable RLS` 를 누르면 안 된다).

### `이메일 보내기` 무반응 — 코드가 아니라 `mailto:` 핸들러 (ADR-048)

사용자 제보. 제일 먼저 한 것은 **코드를 고치는 게 아니라 렌더 결과를 재는 것**이었다 —
프로덕션 HTML 에 `href="mailto:wituskr@gmail.com"` 이 정확히 나오고, `ButtonAnchor` 는
순수 `<a>` 고, `.reveal` 은 `pointer-events` 를 안 건드리고, 서버 컴포넌트라 JS 자체가
안 붙는다. **사이트는 정상이고 받을 핸들러가 없는 것이었다.**

그런데 여기서 끝낼 수 없었다. `company.tel` 이 `null` 이라 창구가 이메일 하나뿐인데,
그 하나가 **실패 신호도 없이** 죽으면 방문자에게 남는 대안이 0이다.

`mailto:` 는 그대로 두고 옆에 `주소 복사` 를 붙였다(`components/copy-email.tsx`).
메일 앱이 뜨는 환경에서는 `mailto:` 가 여전히 빠르므로 **대체가 아니라 병행**이다.
Gmail 웹 작성 링크는 벤더 종속이라 쓰지 않았다 — Gmail 사용자가 아니면 또 조용히 실패한다.

`<a>` 가 아니라 `<button>` 으로 만들면서 `ui.tsx` 에서 `buttonClass()` 를 뽑아
`ButtonLink`·`ButtonAnchor` 와 스타일 정의를 한 곳으로 합쳤다. 복사 실패는 삼키지 않고
라벨(`복사할 수 없습니다`)로 드러낸다. 비보안 컨텍스트용 `execCommand` 폴백도 뒀다.

검증: Playwright 로 실제 클릭 → 클립보드에 `wituskr@gmail.com` 이 들어가고 라벨이
`복사했습니다` → 2.4초 후 복귀. `mailto:` 앵커는 그대로. 390px 가로 스크롤 없음.
`/contact` 는 `○ (Static)` 유지 — 클라이언트 컴포넌트가 하나 늘었지만 동적 렌더로
넘어가지 않았다.

곁가지로 `site-footer.tsx` 가 `mailHref` 를 안 쓰고 `mailto:` 를 직접 만들고 있던 것을
통일했다. 같은 파생값이 두 군데서 만들어지면 한쪽만 바뀐다.
