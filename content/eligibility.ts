 ...
 *   잔액이 아니다.
 *
 * ⚠️ **남은 미해결** — 법 제2조14호의 "대통령령으로 정하는 특수관계인" 범위.
 *
 *    시행령에 **법 제2조14호를 받는 위임 규정이 없다.** 시행령이 "특수관계인" 을 정의하는
 *    곳은 제18조③1호 한 군데인데, 그건 **법 제31조①5호의 주요출자자**(등록 요건·결격사유)
 *    맥락이고 「금융회사의 지배구조에 관한 법률 시행령」 제3조①을 준용한다.
 *
 *    즉 두 가지 중 하나다 —
 *    (a) 시행령이 아직 위임을 이행하지 않았다, 또는
 *    (b) 영 제18조③의 정의를 법 제2조14호에도 끌어 쓴다.
 *
 *    **(b) 라면 계열회사가 특수관계인에 들어가므로 계열사에서 쓰이는 포인트는
 *    "발행인 외의 제3자" 가 아니게 되어 선불전자지급수단이 아니다.** 판정이 뒤집힌다.
 *
 *    → 확인 전까지 `prepaidReach` 안내로 "모르겠다" 를 유도한다.
