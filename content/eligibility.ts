 ...
 *
 * ⚠️ `affiliate` 는 **답이 없어서 있는 선택지다.** 법 제2조14호는 "발행인(대통령령으로
 *    정하는 특수관계인을 포함한다) 외의 제3자" 라고 하는데, 그 특수관계인을 정하는
 *    위임 규정을 시행령에서도 고시에서도 찾지 못했다(2026-09-14).
 *
 *    이 선택지가 없으면 계열사에서만 쓰이는 포인트가 `others` 로 떨어져
 *    **정의 문언에 대응한다고 나온다** — 없는 의무를 있다고 말하는 쪽이다.
 *    당국 질의로 확정되기 전까지는 `unsure` 로 받고, 화면에 그 이유를 적는다.
 */
export type Reach = 'self' | 'sameOwner' | 'affiliate' | 'others' | 'unsure'
...
export const questions: readonly Question[] = [
  ...
  },
  {
    ...
    help:
      ...
      '계열사에서만 쓰이는 경우는 별도 선택지를 골라 주세요 — ' +
      '법 제2조제14호가 말하는 “특수관계인” 의 범위를 정한 규정을 확인하지 못해 대조하지 않습니다.',
    onlyIf: '충전식 잔액을 발행하는 경우',
    choices: [
      { value: 'self', label: '발행사인 우리에게만 쓴다' },
      { value: 'sameOwner', label: '사업주가 같은 가맹점 한 곳에서만 쓴다' },
      { value: 'affiliate', label: '계열사·특수관계인에서만 쓴다' },
...
export function evaluate(a: Answers): Verdict[] {
  ...
  // ── 선불전자지급수단 ─────────────────────────────────────────────────────
  {
    ...
    } else if (a.prepaidReach === 'affiliate') {
      // 위임 규정을 못 찾은 자리. 추정해서 대조하지 않는다.
      level = 'unsure'
      note =
        '계열사·특수관계인에서만 쓰인다고 답하셨습니다. 정의는 “발행인(대통령령으로 정하는 특수관계인을 ' +
        '포함한다) 외의 제3자” 라고 하는데, 그 특수관계인의 범위를 정한 위임 규정을 시행령에서도 ' +
        '고시에서도 찾지 못했습니다(2026. 9. 14. 기준). 범위에 들면 “발행인 외의 제3자” 가 아니게 되어 ' +
        '선불전자지급수단이 아닙니다. 추정하지 않고 확인 필요로 둡니다.'
