...
export const features: Features = {
   ...
   *      **완료**(2026-09-14) — Vercel 환경변수 등록 + 재배포까지 사용자가 마쳤다.
   *      ⚠️ 아직 **종단 확인은 안 됐다.** 폼이 꺼져 있어 접수를 발생시킬 수 없다.
   *        이 플래그를 켠 직후 **테스트 문의를 1건 넣어 Slack 수신을 확인**할 것.
   *        `lib/notify/slack.ts` 는 실패해도 throw 하지 않는다(접수는 살린다) —
   *        즉 **웹훅이 틀려도 화면상 증상이 없다.** 눈으로 확인하는 수밖에 없다.
