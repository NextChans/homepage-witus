...
export const features: Features = {
   ...
   *      배선 확인은 **`/admin` 의 "접수 알림 채널 → 테스트 메시지 보내기"** 로 한다
   *      (관리자 전용). `lib/notify/slack.ts` 는 실패해도 throw 하지 않으므로
   *      **웹훅이 틀려도 화면상 증상이 없다** — 눌러서 확인하는 수밖에 없다.
   *      ⚠️ 환경변수를 바꾸거나 웹훅을 재발급했으면 **재배포 후 다시 누른다.**
