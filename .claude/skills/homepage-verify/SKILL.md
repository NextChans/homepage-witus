...
### 1-1. 플레이스홀더 누출 검사 (**커스텀 도메인이 붙은 뒤로는 필수**)

`witus.kr` 이 연결된 뒤로 `content/` 의 더미 값은 더 이상 더미가 아니라 **허위 표시**다.
실제로 2026-09-13 시점 홈 한 페이지에 `02-0000-0000` 4회, `000-00-00000` 2회,
`테헤란로 000` 2회가 나가고 있었다(ADR-028).

```bash
# 빌드된 서버를 띄운 뒤 주요 페이지를 받아 한 번에 센다.
for u in / /about /contact /services; do curl -sS "http://localhost:3100$u"; done > /tmp/pages.html
for pat in '0000-0000' '000-00-00000' '테헤란로 000' 'XXX+' 'X,XXX' '템플릿 예시'; do
  echo "$pat: $(grep -o "$pat" /tmp/pages.html | wc -l)"
done
```

**전부 0 이어야 한다.** 0 이 아니면 값을 만들어 채우지 말고 —
- 회사 정보라면 `content/site.ts` 에서 **`null`** 로 둔다 (화면이 줄째로 접는다, ADR-028)
- 지표·실적이라면 `content/features.ts` 의 플래그로 **내린다**

⚠️ `grep -o | wc -l` 을 쓴다. `grep -c` 는 **줄 수**를 센다 (함정 8).
...
### 함정 7-1 — **본문과 상태코드를 두 번 요청해 받으면 서로 다른 응답을 본다**

이런 스크립트를 쓰면 안 된다. 2026-09-14 에 **두 번** 같은 증상이 나왔다 —
`/contact 200 본문 0 B`, `/privacy 404 본문 0 B`. 둘 다 재측정하니 정상이었다.

```bash
# ✗ curl 을 두 번 부른다 — 배포 전환 중이면 둘이 다른 응답을 본다
body=$(curl -sS "$URL")
code=$(curl -sS -o /dev/null -w '%{http_code}' "$URL")
```

배포 직후 확인은 Vercel 이 트래픽을 전환하는 중이라 **요청 사이에 상태가 바뀐다.**
"코드는 정상인데 본문이 0 B" 는 **회귀처럼 보이지만 측정 결함**이다 — 그리고
ADR-038 의 "빈 값을 정상 데이터로 읽는다" 와 **정반대 방향의 오판**(정상을 장애로
읽는다)이라 더 헷갈린다.

```bash
# ✓ 한 번의 요청에서 코드와 크기를 함께 받는다
out=$(curl -sS -w '\n__C%{http_code} __S%{size_download}' "$URL")
meta=$(printf '%s' "$out" | tail -1)
body=$(printf '%s' "$out" | sed '$d')
```

⚠️ **그래도 0 B 가 나오면 같은 방식으로 3회 반복**한다. 한 번의 결과로 판단하지
   않는다 — 진짜 회귀라면 3회 모두 같게 나온다.
...
- [ ] **플래그로 숨긴 섹션의 빈 래퍼가 남아 있지 않다.** 컴포넌트가 `null` 을
      반환해도 **호출부의 배경·구분선 래퍼는 그대로 렌더된다** — 높이 1px 짜리
      띠가 남아 hairline 이 두 줄로 겹친다(ADR-035, 실제로 발생). 눈으로는 거의
      안 보이니 DOM 을 잰다:

      ```js
      // 결과가 0 이어야 한다
      await p.evaluate(() => [...document.querySelectorAll('div.border-t')]
        .filter(el => el.children.length === 0 && !el.textContent.trim()).length)
      ```

      → 발견되면 **래퍼를 컴포넌트 안으로 옮긴다.** 숨길 수 있는 섹션은
        자기 크롬을 자기가 소유해야 깨끗하게 사라진다.
- [ ] **푸터에 `템플릿 예시` 문구가 없다.** ⚠️ 예전 체크리스트는 반대였다 —
      도메인이 붙기 전 이야기다. 실제 사명이 들어간 지금 그 문장은 자기 사이트를
      가짜라고 선언한다(ADR-028). 지금 있어야 할 것은 **자문 아님 고지**다
- [ ] **사업자 표시 줄에 확정된 항목만 있다.** 사업자등록번호·주소가 `null` 이면
      그 조각이 통째로 빠지고 `· ·` 같은 빈 구분자가 남지 않는다
- [ ] **`/contact` 히어로가 실제 연락 수단과 맞는다.** 전화가 `null` 인데
      `전화 한 통이면 됩니다.` 가 보이면 잘못된 것이다
